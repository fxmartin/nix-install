...
setup_security_report() {
    ...
    if [[ -f "$REPORT_SCRIPT" ]] && [[ -f "$REPORT_CONFIG" ]]; then
        log_ok "Security report already configured"
        return 0
    fi
    ...
    local smtp_host smtp_port smtp_user smtp_pass report_email
    ...
    if [[ -z "$smtp_pass" ]]; then
        log_warn "No password provided - skipping security report setup"
        log_warn "Run this script again or manually configure /root/.msmtprc"
        return 0
    fi
    ...
    # We need to create a wrapper that sources the Nix environment
...
REPORTEOF
    ...
    # Setup cron job (7am Europe/Paris)
    log_info "Setting up cron job (7am Europe/Paris)..."
    local CRON_JOB="0 7 * * * TZ=Europe/Paris ${REPORT_SCRIPT} >/dev/null 2>&1"
