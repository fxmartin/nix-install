...
# ABOUTME: Secrets (Kindle serial, Adobe keys) loaded from ~/.config/calibre-secrets/
{
  config,
  pkgs,
  lib,
  ...
  # - Sensitive data (Kindle serial, Adobe keys): ~/.config/calibre-secrets/ (local only)
  ...
  system.activationScripts.postActivation.text = lib.mkAfter ''
    ...
    if [[ -d "$CALIBRE_SRC" ]]; then
      ...
      if [[ -d "$CALIBRE_SECRETS" ]]; then
        ...
        echo "  Then add dedrm.json and DeACSM/account/ files"
      fi
