...
# ABOUTME: Scripts are installed to ~/.local/bin to avoid macOS TCC restrictions
{
  ...
  generateJobsConf = config: let
    # Convert a job to the pipe-delimited format: name|source|destination|excludes
    jobToLine = job:
      "${job.name}|${job.source}|${job.destination}|${lib.concatStringsSep "," job.excludes}";
  ...
  in ''
    ...
    # Backup jobs (pipe-delimited: name|source|destination|excludes)
    JOBS=(
