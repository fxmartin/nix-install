...
- **starship** - Cross-shell prompt with 2-line lean style
  - **Module**: `home-manager/modules/shell.nix` (inline in `programs.starship.settings`)
  - **Config Location**: `~/.config/starship.toml` (generated by Home Manager)
  - **Features**: OS icon, directory, git status, right prompt with languages/cloud/tools
  - **Note**: Configuration is fully inline in Nix - no separate TOML file needed
