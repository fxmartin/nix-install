...
mlx-whisper is 2-3x faster than whisper.cpp on Apple Silicon (M3 Max), leveraging the MLX framework for native Metal acceleration.
...
4. **ffmpeg**: Installed via Homebrew (`/opt/homebrew/bin/ffmpeg`) for audio format conversion
...
pip install fastapi uvicorn mlx-whisper python-multipart
```
...
- **Environment**: `HOME` set to user home, `PATH` includes `/opt/homebrew/bin` (for ffmpeg)
