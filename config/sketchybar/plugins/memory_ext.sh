...
USED_GB=${MEM_USED:-0}
PCT=$(awk -v u="${USED_GB}" -v t="${MEM_TOTAL}" 'BEGIN { if (t > 0) printf "%.0f", (u/t)*100; else print 0 }')
...
case "${NAME}" in
  memory.short) LABEL="${BAR} ${PCT}% U${USED_GB}G S${SWAP_USED}G" ;;
  *)            LABEL="${BAR} ${PCT}% U${USED_GB}G A${MEM_ACTIVE}G I${MEM_INACTIVE}G S${SWAP_USED}G" ;;
esac
