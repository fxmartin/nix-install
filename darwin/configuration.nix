...
in
{
  ...
  #
  # Not Power-gated: the llama.cpp brew ships on every profile (homebrew.nix
  # base brews), so every Mac needs its cache under the swept root.
  #
  # OMLX_MODEL_DIR only reaches shell-launched tools — the oMLX menubar app is
  # launched by launchd, which never sources /etc/zshenv. The app's
  # authoritative setting is ~/.omlx/settings.json, patched to the same path
  # by darwin/omlx.nix.
  #
  ...
  // lib.optionalAttrs isPowerProfile {
