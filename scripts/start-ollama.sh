...
# Without this, ollama serve silently uses ~/.ollama/models on the internal
# disk and the mounted-volume checks above protect nothing.
export OLLAMA_MODELS="${OLLAMA_MODELS:-$MODEL_DIR}"

/bin/launchctl remove "$LAUNCH_LABEL" >/dev/null 2>&1 || true
/bin/launchctl submit -l "$LAUNCH_LABEL" -- /bin/bash -lc \
  "OLLAMA_HOST='$OLLAMA_HOST' OLLAMA_ORIGINS='$OLLAMA_ORIGINS' OLLAMA_MAX_LOADED_MODELS='$OLLAMA_MAX_LOADED_MODELS' OLLAMA_NUM_PARALLEL='$OLLAMA_NUM_PARALLEL' OLLAMA_KEEP_ALIVE='$OLLAMA_KEEP_ALIVE' OLLAMA_MODELS='$OLLAMA_MODELS' exec '$OLLAMA_BIN' serve >>'$LOG_FILE' 2>>'$ERR_FILE'"
